...
if __name__ == "__main__":
    ...
    wass_times_t = main(tvAR_type, times_t, times_T, d, n_replications, space_kernel, time_kernel, zeta, C, path_result)
    print("Done!")
    print(wass_times_t)
    ...
    # for t in times_t:
    #     wass_distance_time_t_means[str(t)] = [wass_distance_mcs[str(m)][f"t:{t}"] for m in range(M)]
    #
    #
    # plt.rcParams["figure.figsize"] = (8, 4)
    # colors = plt.cm.Set1(np.linspace(0, .5, len(times_t)))
    ...
    #     plt.tight_layout()
    #     plt.savefig(
    #         path_result + f"wass_distance{times_t}{times_T}_{n_replications}_spkernel_{space_kernel}_timekernel_{time_kernel}_zeta{zeta}_C{C}.pdf",
    #         dpi=150)
