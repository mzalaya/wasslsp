...
"""
Gaussian tvAR(2)
Ref: Dahlhaus, R. (2012). Locally stationary processes. In Rao, T. S., Rao, S. S., and Rao, C., editors, 
Time Series Analysis: Methods and Applications, volume 30 of Handbook of Statistics, pages 351
"""
...
@njit
def m_star(u, x, y):
    return psi_star(u) * np.cos(phi_star(u)) * x - 0.81 * y
...
def plot_m_star(process, T_samples):
    ...
    m_star_vals = np.array([m_star(u_val, x_val, x_val) for u_val, x_val in zip(u, x)])
...
def simulation_1_rep_process(d, T):
    ...
    while t <= T - 1:
        m_star_val = m_star(t / T, X_tvar_2_T[t - 1], X_tvar_2_T[t - 2])
...
def plot_1_rep_process(process, d, T_samples, device):
    ...
    X_tvar_2_replications = torch.zeros((1, T_samples)).to(device)  # For 1 replication
    X_dict = {}
    ...
    # Save results
    X_tvar_2 = torch.tensor(X_tvar_2_np).to(device)
    X_dict["0"] = X
    X_tvar_2_replications[0] = X_tvar_2

    # Plotting
    fig = plt.figure(figsize=(10, 3))
    plt.grid(which="major", color="#DDDDDD", linewidth=0.8)
    plt.minorticks_on()
    plt.plot(X_tvar_2_replications[0].detach().cpu().numpy(), lw=1, label="Replication 1")
    plt.xlim(0, T_samples)
    plt.ylim(
        min(X_tvar_2_replications[0].detach().cpu().numpy()),
        max(X_tvar_2_replications[0].detach().cpu().numpy())
    )
...
def simulation_L_rep_process(d, times_t_dict, times_T, n_replications, device, output_dir, iterations):
    ...
    # Initialize variables
    X_tvar_2_iteration = None
    X_tvar_2_replications = {f"T:{T}": torch.zeros((n_replications, T), dtype=torch.float16).to(device) for T in times_T}
    ...
    for iteration in range(iterations):
        ...
        for T in times_T:
            for replication in range(n_replications):
                X_tvar_2_T, X = simulation_1_rep_process(d, T)
                X_tvar_2_replications[f"T:{T}"][replication] = torch.from_numpy(X_tvar_2_T).to(device)
        ...
        for T in times_T:
            ...
            for t in times_t:
                ...
                data = [X_tvar_2_replications[f"T:{T}"][replication][t - 1].item()
                        for replication in range(n_replications)]
                X_tvar_2[f"t:{t}_T:{T}"][:] = torch.tensor(data, dtype=torch.float16, device=device)

        # Save the current iteration's results to disk using pickle
        iteration_results = {
            "X_tvar_2_replications": X_tvar_2_replications,
            "X_dict": X_dict,
            "X_tvar_2_iteration": X_tvar_2  
        }
        ...
        # Keep the last iteration's X_tvar_2 for returning
        X_tvar_2_iteration = X_tvar_2

        # Release memory after saving
        del X_tvar_2_replications, X_dict, iteration_results
    ...
    if X_tvar_2_iteration is not None:
        return X_tvar_2_iteration, {f"T:{T}": torch.zeros((n_replications, T), dtype=torch.float16).to(device) for T in times_T}, {f"T:{T}": {} for T in times_T}
    else:
        raise ValueError("X_tvar_2 was not properly assigned during the iterations.")
...
def empirical_cdf(times_t_dict, times_T, device, iterations, input_dir, X_tvar_2):
    ...
    for iteration in range(iterations):
        ...
        X_tvar_2 = iteration_results["X_tvar_2_iteration"]

        empirical_cdfs = TensorDict(
            {
                f"t:{t}_T:{T}": ECDFTorch(X_tvar_2[f"t:{t}_T:{T}"]).y
                for T in times_T for t in times_t_dict[T]  
            },
            device=device,
        )
...
def wasserstein_distances(times_t_dict, times_T, n_replications, iterations, gaussian_weights, empirical_cdfs_iterations, X_tvar_2_replications, 
    ...
    for iteration in range(iterations):
        ...
        # Load the iteration results for the current iteration to access X_tvar_2_replications
        with open(os.path.join(input_dir, f"iteration_{iteration + 1}.pkl"), "rb") as f:
            iteration_results = pickle.load(f)
    
        X_tvar_2_replications = iteration_results["X_tvar_2_replications"]
...
def main():
    ...
    wass_distances_empirical_meanNW_iterations = wasserstein_distances(times_t_dict, times_T, n_replications, iterations, gaussian_weights, 
                                                                       empirical_cdfs_iterations, X_tvar_2_replications, input_dir, input_dir_weights, 
                                                                       output_dir, device, pplot=None)
    
    ### Mean Wasserstein distance
    input_dir = "GaussiantvAR2_Wass"
    output_dir = "GaussiantvAR2WassStats"
    os.makedirs(output_dir, exist_ok=True)
    ...
    input_dir = "GaussiantvAR2WassStats"
