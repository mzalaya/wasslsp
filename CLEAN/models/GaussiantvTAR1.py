...
@njit
def phi1(u):
    return 0.4 * np.sin(2 * np.pi * u)

@njit
def phi2(u):
    return 0.5 * np.cos(2 * np.pi * u)

@njit
def m_star(u, x):
    return phi1(u) * max(x, 0) + phi2(u) * max(-x, 0)
...
def simulation_1_rep_process(d, T):
    ...
    while t <= T - 1:
        m_star_val = m_star(t / T, X_tvtar_1_T[t - 1])
        X_tvtar_1_T[t] = m_star_val + epsilon[t]  
        X[t] = np.array([X_tvtar_1_T[t - 1]])
        t += 1
...
def plot_1_rep_process(process, d, T_samples, device):
    ...
    X_tvtar_1_replications = torch.zeros((1, T_samples)).to(device)  # For 1 replication
    X_dict = {}

    torch.manual_seed(31)  
    X = torch.zeros((T_samples, d)).to(device)  
    X_tvtar_1_np = np.zeros(T_samples)  

    X_tvtar_1_np, X = simulation_1_rep_process(d, T_samples)

    # Save results
    X_tvtar_1 = torch.tensor(X_tvtar_1_np).to(device)
    X_dict["0"] = X
    X_tvtar_1_replications[0] = X_tvtar_1

    # Plotting
    fig = plt.figure(figsize=(10, 3))
    plt.grid(which="major", color="#DDDDDD", linewidth=0.8)
    plt.minorticks_on()
    plt.plot(X_tvtar_1_replications[0].detach().cpu().numpy(), lw=1, label="Replication 1")
    plt.xlim(0, T_samples)
    plt.ylim(
        min(X_tvtar_1_replications[0].detach().cpu().numpy()),
        max(X_tvtar_1_replications[0].detach().cpu().numpy())
    )
...
def simulation_L_rep_process(d, times_t_dict, times_T, n_replications, device, output_dir, iterations):
    ...
    # Initialize variables 
    X_tvtar_1_iteration = None
    X_tvtar_1_replications = {f"T:{T}": torch.zeros((n_replications, T), dtype=torch.float16).to(device) for T in times_T}
    ...
    for iteration in range(iterations):
        ...
        for T in times_T:
            for replication in range(n_replications):
                X_tvtar_1_T, X = simulation_1_rep_process(d, T)
                X_tvtar_1_replications[f"T:{T}"][replication] = torch.from_numpy(X_tvtar_1_T).to(device)
        ...
        for T in times_T:
            ...
            for t in times_t:
                ...
                data = [X_tvtar_1_replications[f"T:{T}"][replication][t - 1].item()
                        for replication in range(n_replications)]
                X_tvtar_1[f"t:{t}_T:{T}"][:] = torch.tensor(data, dtype=torch.float16, device=device)

        # Save the current iteration's results to disk using pickle
        iteration_results = {
            "X_tvtar_1_replications": X_tvtar_1_replications,
            "X_dict": X_dict,
            "X_tvtar_1_iteration": X_tvtar_1  
        }
        ...
        # Keep the last iteration's X_tvtar_1 for returning
        X_tvtar_1_iteration = X_tvtar_1

        # Release memory after saving
        del X_tvtar_1_replications, X_dict, iteration_results
    ...
    if X_tvtar_1_iteration is not None:
        return X_tvtar_1_iteration, {f"T:{T}": torch.zeros((n_replications, T), dtype=torch.float16).to(device) for T in times_T}, {f"T:{T}": {} for T in times_T}
    else:
        raise ValueError("X_tvtar_1 was not properly assigned during the iterations.")
...
def empirical_cdf(times_t_dict, times_T, device, iterations, input_dir, X_tvtar_1):
    ...
    for iteration in range(iterations):
        ...
        X_tvtar_1 = iteration_results["X_tvtar_1_iteration"]

        empirical_cdfs = TensorDict(
            {
                f"t:{t}_T:{T}": ECDFTorch(X_tvtar_1[f"t:{t}_T:{T}"]).y
                for T in times_T for t in times_t_dict[T]  
            },
            device=device,
        )
...
def wasserstein_distances(times_t_dict, times_T, n_replications, iterations, gaussian_weights, empirical_cdfs_iterations, X_tvtar_1_replications, 
    ...
    for iteration in range(iterations):
        ...
        # Load the iteration results for the current iteration to access X_tvtar_1_replications
        with open(os.path.join(input_dir, f"iteration_{iteration + 1}.pkl"), "rb") as f:
            iteration_results = pickle.load(f)
    
        X_tvtar_1_replications = iteration_results["X_tvtar_1_replications"]
            ...
            for T in times_T:
                ...
                for t in times_t:
                    ...
                    weighted_ecdf = ECDFTorch(
                        X_tvtar_1_replications[f"T:{T}"][replication],
...
def main():
    ...
    ### Generating L replications of LSPs
    ...
    X_tvtar_1, X_tvtar_1_replications, X_dict = simulation_L_rep_process(d, times_t_dict, times_T, n_replications, device, output_dir, iterations)
    ...
    input_dir = "simulation_results"
    empirical_cdfs_iterations = empirical_cdf(times_t_dict, times_T, device, iterations, input_dir, X_tvtar_1)

    ### Wasserstein distances
    input_dir = "simulation_results"
    input_dir_weights = "gaussian_weights_output"
    output_dir = "tvTAR1_Wass"
    os.makedirs(output_dir, exist_ok=True)
    ...
    wass_distances_empirical_meanNW_iterations = wasserstein_distances(times_t_dict, times_T, n_replications, iterations, gaussian_weights, 
                                                                       empirical_cdfs_iterations, X_tvtar_1_replications, input_dir, input_dir_weights, 
                                                                       output_dir, device, pplot=None)
    
    ### Mean Wasserstein distance
    input_dir = "tvTAR1_Wass"
    output_dir = "tvTAR1WassStats"
    os.makedirs(output_dir, exist_ok=True)
