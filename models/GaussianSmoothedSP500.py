...
def plot_sample_noised(data, T, sigma_fix, n_replications_fix):
    ...
    plt.tight_layout()
    plt.savefig(f"{n_replications_fix} replications of SP500 sigma={sigma_fix}.pdf", dpi=300)
    plt.show()
...
def plot_nw_cdf_estimators(x_rep, y_rep, t_fixed, sigma_fix, n_replications_fix, T, iterations, input_dir_weights, input_dir):
    ...
    for iteration in range(iterations):
        ...
        for replication in range(n_replications_fix):
            for sigma in sigma_fix:
                for t in t_fixed:
                    ...
                    plt.xlabel("SP500 values", fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig(f"{n_replications_fix} NW estimators of SP500 sigma={sigma_fix}.pdf", dpi=300)
        plt.show()
