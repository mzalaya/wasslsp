...
def simulation_1_rep_process(d, T, device):
    ...
    epsilon = cauchy_distrib.sample(sample_shape=(T,)).to(device)
...
def main():
    ...
    wass_times_t = wasserstein_distances(times_t, times_T, n_replications, X_tvar_2_replications, gaussian_weights_tensor, empirical_cdf_vals, process, time_kernel, space_kernel, path_dicts, device)
